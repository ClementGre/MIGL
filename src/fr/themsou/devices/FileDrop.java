           ...
           try{
              if(type.equals(DataFlavor.javaFileListFlavor)){
                 ...
                 if(getFileExtension(file).equals("jpg") || getFileExtension(file).equals("png") || getFileExtension(file).equals("jpeg") || getFileExtension(file).equals("tif")){
                	...
                	int mouseX = MouseInfo.getPointerInfo().getLocation().x - Main.render.getLocationOnScreen().x;
 		  			int mouseY = MouseInfo.getPointerInfo().getLocation().y - Main.render.getLocationOnScreen().y;
 		  			
 		  			int x = (int) ((mouseX - (Main.render.getWidth() - RenderClass.width) / 2) / RenderClass.squareSize);
 		  			int y = (int) ((mouseY - (Main.render.getHeight() - RenderClass.width) / 2) / RenderClass.squareSize);
 					Main.render.addFile(file, x, y);
